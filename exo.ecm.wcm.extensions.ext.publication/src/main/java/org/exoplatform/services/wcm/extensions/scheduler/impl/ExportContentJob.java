...
public class ExportContentJob implements Job {
	...
	try {
		...
		for (NodeIterator iter = queryResult.getNodes(); iter.hasNext();) {
		    ...
		    HashMap<String, String> publicationContext = new HashMap<String, String>();
		    publicationContext.put(StageAndVersionPublicationConstant.CURRENT_REVISION_NAME, node_.getName());
		    session.save();
		    publicationService_.changeState(node_, toState, publicationContext);
	    ...
	    log.error("error when exporting content");
	} finally {
	    session.logout();
	}
