...
public class UISearchPageLayoutManager extends UIForm {
	...
	public UISearchPageLayoutManager() throws Exception {
		...
		String searchFormTemplate = portletPreferences.getValue(
				UIWCMSearchPortlet.SEARCH_FORM_TEMPLATE_PATH, null);
		String searchResultTemplate = portletPreferences.getValue(
				UIWCMSearchPortlet.SEARCH_RESULT_TEMPLATE_PATH, null);
		String searchPaginatorTemplate = portletPreferences.getValue(
				UIWCMSearchPortlet.SEARCH_PAGINATOR_TEMPLATE_PATH, null);
		String searchPageLayoutTemplate = portletPreferences.getValue(
				UIWCMSearchPortlet.SEARCH_PAGE_LAYOUT_TEMPLATE_PATH, null);
		String searchBoxTemplate = portletPreferences.getValue(
				UIWCMSearchPortlet.SEARCH_BOX_TEMPLATE_PATH, null);
		String searchMode = portletPreferences.getValue(
				UIWCMSearchPortlet.SEARCH_MODE, null);

		List<SelectItemOption<String>> searchModeList = createSearchModeList();
		List<SelectItemOption<String>> searchFormTemplateList = createTemplateList(
				PORTLET_NAME, SEARCH_FORM_TEMPLATE_CATEGORY);
		List<SelectItemOption<String>> searchResultTemplateList = createTemplateList(
				PORTLET_NAME, SEARCH_RESULT_TEMPLATE_CATEGORY);
		List<SelectItemOption<String>> searchPaginatorTemplateList = createTemplateList(
				PORTLET_NAME, SEARCH_PAGINATOR_TEMPLATE_CATEGORY);
		List<SelectItemOption<String>> searchBoxTemplateList = createTemplateList(
				PORTLET_NAME, SEARCH_BOX_TEMPLATE_CATEGORY);
		List<SelectItemOption<String>> searchPageLayoutTemplateList = createTemplateList(
				PORTLET_NAME, SEARCH_PAGE_LAYOUT_CATEGORY);
		...
		UIFormSelectBox searchModeSelector = new UIFormSelectBox(
				SEARCH_MODE_SELECTOR, SEARCH_MODE_SELECTOR, searchModeList);
		UIFormSelectBox itemsPerPageSelector = new UIFormSelectBox(
				ITEMS_PER_PAGE_SELECTOR, ITEMS_PER_PAGE_SELECTOR, itemsPerPageList);
		UIFormSelectBox searchFormTemplateSelector = new UIFormSelectBox(
				SEARCH_FORM_TEMPLATE_SELECTOR, SEARCH_FORM_TEMPLATE_SELECTOR,
		...
		} else if (UISearchPageLayoutManager.SEARCH_PAGE_MODE_OPTION
				.equals(searchMode)) {
	...
	private List<SelectItemOption<String>> createTemplateList(String portletName,
		...
		List<Node> templateNodeList = templateManagerService
				.getTemplatesByCategory(repository, portletName, category, provider);
		for (Node templateNode : templateNodeList) {
			String templateName = templateNode.getName();
			String templatePath = templateNode.getPath();
			templateList
					.add(new SelectItemOption<String>(templateName, templatePath));
		}
		return templateList;
	}
	...
	private List<SelectItemOption<String>> createSearchModeList()
		...
		searchModesList.add(new SelectItemOption<String>(modesLabel,
				SEARCH_MODES_OPTION));
		searchModesList.add(new SelectItemOption<String>(boxModeLabel,
				SEARCH_BOX_MODE_OPTION));
		searchModesList.add(new SelectItemOption<String>(pageModeLabel,
				SEARCH_PAGE_MODE_OPTION));
	...
	public static class SaveActionListener extends
		...
		public void execute(Event<UISearchPageLayoutManager> event)
			...
			UIApplication uiApp = uiSearchLayoutManager
			...
			String searchResultTemplatePath = uiSearchLayoutManager
					.getUIFormSelectBox(
							UISearchPageLayoutManager.SEARCH_RESULT_TEMPLATE_SELECTOR)
					.getValue();
			...
			String searchPaginatorTemplatePath = uiSearchLayoutManager
					.getUIFormSelectBox(
							UISearchPageLayoutManager.SEARCH_PAGINATOR_TEMPLATE_SELECTOR)
					.getValue();
			...
			if (Utils.isEditPortletInCreatePageWizard()) {
				uiApp
						.addMessage(new ApplicationMessage(
								"UIMessageBoard.msg.saving-success", null,
	...
	public static class SelectSearchModeActionListener extends
		...
		public void execute(Event<UISearchPageLayoutManager> event)
			...
			event.getRequestContext().addUIComponentToUpdateByAjax(
					uiSearchPageLayoutManager.getParent());
		}
	}
