				...
				if(httpRequest.status == 200){
					...
					if(list && list.length > 0){
						...
						parentPath = document.getElementById(
													objChild.id.replace(eXo.wcm.WCMUtils.UIWCMTreeView.constChildNode, 
																		eXo.wcm.UIWCMTreeView.constParentNode)
						...
						for(var i = 0; i < list.length; i ++){;
							...
							firstId = eXo.wcm.UIWCMTreeView.replaceAll(
														eXo.wcm.UIWCMTreeView.replaceAll(treePath, "/", "rp"), " ", "");
							restAction = "eXo.wcm.UIWCMTreeView.selectNode('"+firstId+"','"+nodePath+"', '"+workspaceName+"', '"+repositoryName+"');" + 
														eXo.wcm.UIWCMTreeView.ajaxAction.replace("WCMTreeBuilderParameters",
														eXo.wcm.UIWCMTreeView.replaceAll(eXo.wcm.UIWCMTreeView.replaceAll(
																																nodePath+"/"+workspaceName+"/"+firstId,"/", "%2F"), " ", "+"));
																																
							objChild.innerHTML += '<div class="NoneSelectNodeIcon" onclick="' + restAction + '" id="'+
																		eXo.wcm.UIWCMTreeView.constParentNode + firstId+'" treePath="'+treePath+'">'+
																		'<div class="DefaultPageIcon">'+name+'</div>'+
																		'</div>'+
																		'<div class="ViewChildNodes" id="'+eXo.wcm.UIWCMTreeView.constChildNode + firstId+'"></div>';
						}
					}
...
UIWCMTreeView.prototype.selectNode = function(childId, nodePath, workspaceName, repositoryName) {
	var objParent = document.getElementById(eXo.wcm.UIWCMTreeView.constParentNode + childId);
	if(objParent){
		...
		if(eXo.wcm.UIWCMTreeView.oldId != (eXo.wcm.UIWCMTreeView.constParentNode + childId)){
			oldObject = document.getElementById(eXo.wcm.UIWCMTreeView.oldId);
			if(oldObject) oldObject.style.fontWeight="normal";
			eXo.wcm.UIWCMTreeView.oldId = eXo.wcm.UIWCMTreeView.constParentNode + childId;
			eXo.wcm.UIWCMTreeView.updateBreadcumbsElement(objParent);
		}
	}
	
	var objChild = document.getElementById(eXo.wcm.UIWCMTreeView.constChildNode + childId);
...
UIWCMTreeView.prototype.updateBreadcumbsElement = function(objParent) {
	...
	for(var i = nodeNames.length - 1; i >=0; i --){
		...
		newDiv.id = eXo.wcm.UIWCMTreeView.constBreadcumbsElement + i;
		...
		brotherDiv = document.getElementById(eXo.wcm.UIWCMTreeView.constBreadcumbsElement + (i + 1));
		...
		if(objParent && objParent != null && objParent.id && objParent.id != null && objParent.id.indexOf(eXo.wcm.UIWCMTreeView.constChildNode) > -1){
			objParentId = objParent.id.replace(eXo.wcm.UIWCMTreeView.constChildNode, eXo.wcm.UIWCMTreeView.constParentNode);
			objParent = document.getElementById(objParentId);
		}else{
			objParent = null;
		}
	}
	objParent = document.getElementById(eXo.wcm.UIWCMTreeView.constBreadcumbsElement + (nodeNames.length - 1));
...
UIWCMTreeView.prototype.OpenOnePath = function(id, isLoad){
	...
	if(obj){
		...
		eval(action);
	}else setTimeout('eXo.wcm.UIWCMTreeView.OpenOnePath("'+id+'",'+isLoad+')', 100);
};
